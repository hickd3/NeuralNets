...
class Layer:
    ...
    def softmax(self):
        ...
        No return
        '''
        net_in = self.net_in - np.max(self.net_in, axis=1, keepdims=True)
        expnet_in = np.exp(net_in)
        self.net_act= expnet_in / np.sum(expnet_in, axis=1, keepdims=True)
    ...
    def cross_entropy(self, y):
        ...
        '''
        N = y.shape[0] 
        proba = np.clip(self.net_act, 1e-8, 1)
        loss = -np.log(proba[np.arange(N), y])
        return np.mean(loss) 
    ...
    def backward_netAct_to_netIn(self, d_upstream, y):
        ...
        if self.activation == 'linear':
            ...
            d_net_in = np.where(self.net_in > 0, d_upstream, 0)
        elif self.activation == 'softmax':
            batch_size = d_upstream.shape[0]
            d_net_in = self.net_act.copy()
            d_net_in[np.arange(batch_size), y] -= 1
            d_net_in /= batch_size
        else:
            print("provide an activation function")
        return d_net_in
            ...
            for chan in range(n_chans):
                for j in range(out_y):
                    for i in range(out_x):
                        y_start = j * self.strides
                        x_start = i * self.strides
                        y_end = y_start + self.pool_size
                        x_end = x_start + self.pool_size

                        window = self.input[batch, chan, y_start:y_end, x_start:x_end]
                        max_idx_flat = np.argmax(window)
                        max_y, max_x = self.ind2sub(max_idx_flat, window.shape)

                        dprev_net_act[batch, chan, y_start + max_y, x_start + max_x] += d_upstream[batch, chan, j , i]
...
class Flatten(Layer):
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        '''
        return d_upstream.reshape(self.input.shape), None, None
...
class Dropout(Layer):
    ...
    def __init__(self, number, name, rate=0.5, r_seed=None, verbose=True):
        ...
        self.rate = rate
        self.r_seed = r_seed
        self.rng = np.random.default_rng(self.r_seed)
