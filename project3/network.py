...
class Network:
    ...
    def set_dropout_layers_mode(self, in_train_mode=False):
        ...
        for layer in self.dropout_layer_inds:
            self.layers[layer].set_mode(in_train_mode)
    ...
    def backward(self, y):
        ...
        '''
        self.d_upstream = None
        self.d_wts = None
        self.d_b = None
        for layer in reversed(self.layers):
            self.d_upstream, _, _ = layer.backward(self.d_upstream, y)
    ...
    def predict(self, inputs):
        ...
        mode = []
        for layer in self.dropout_layer_inds:
            mode.append(self.layers[layer].in_train_mode)
            self.layers[layer].set_mode(False)

        for layer in self.layers:
            inputs = layer.forward(inputs)
        pred_classes = np.argmax(inputs, axis=1)
        for layer, ind in enumerate(self.dropout_layer_inds):
            self.layers[ind].set_mode(mode[layer])
    ...
    def accuracy(self, inputs, y, samp_sz=500, mini_batch_sz=15):
        ...
        for batch in range(n_batches):
            low = batch*mini_batch_sz
            high = batch*mini_batch_sz+mini_batch_sz
            if high > n_samps:
                high = n_samps
    ...
    def fit(self, x_train, y_train, x_validate, y_validate, mini_batch_sz=256, n_epochs=1, acc_freq=10, print_every=50):
        ...
        '''
        start = time.time()
        n_samps = x_train.shape[0]
        n_batches = int(np.ceil(n_samps / mini_batch_sz))
        n_iters = n_batches * n_epochs

        self.loss_history = []
        self.train_acc_history = []
        self.validation_acc_history = []
        self.in_train_mode = True
        self.set_dropout_layers_mode(in_train_mode = True)

        for epoch in range(n_epochs):
            rng = np.random.default_rng(epoch)
            idx = rng.permutation(n_samps)
            x_train = x_train[idx]
            y_train = y_train[idx]
            for batch in range(n_batches):
                tot_iter = epoch * n_batches + batch
                low = batch * mini_batch_sz
                high = min((batch + 1) * mini_batch_sz, n_samps)

                x_mini_batch = x_train[low:high]
                y_mini_batch = y_train[low:high]

                loss = self.forward(x_mini_batch, y_mini_batch)
                self.backward(y_mini_batch)

                for layer_idx in self.wt_layer_inds:
                    self.layers[layer_idx].update_weights()

                self.loss_history.append(loss)

                if tot_iter % print_every == 0 and tot_iter != 0:
                    print(f'iteration: {tot_iter}, loss: {loss:.2f}')

                if tot_iter % acc_freq == 0 and tot_iter > 0:
                    train_acc = self.accuracy(x_train, y_train)
                    val_acc = self.accuracy(x_validate, y_validate)
                    self.train_acc_history.append(train_acc)
                    self.validation_acc_history.append(val_acc)
                    print(f'iteration:{tot_iter}, train acc: {train_acc:.2f}, val acc: {val_acc:.2f}')

                    if tot_iter == acc_freq:
                        elapsed_time = (time.time() - start) / 60
                        projected_total = (elapsed_time / tot_iter) * n_iters
                        print(f'Elapsed: {elapsed_time:.2f} mins, Projected Total: {projected_total:.2f} mins')

        self.in_train_mode = False
        self.set_dropout_layers_mode(self.in_train_mode)

        print(f'loss: {self.loss_history[-1]:.2f}')
        print(f'final train acc: {self.train_acc_history[-1]:.2f}')
        print(f'final val acc: {self.validation_acc_history[-1]:.2f}')
...
class ConvNet4Accel(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        prev_layer_sz_conv = (((h - ker_sz[0]) // 1), ((w - ker_sz[0]) // 1))
        prev_layer_sz_pool = (((prev_layer_sz_conv[0] - pooling_sizes[0]) // pooling_strides[0]),
                              ((prev_layer_sz_conv[1] - pooling_sizes[0]) // pooling_strides[0]))
       
        prev_layer = (h // pooling_sizes[0]) * (w // pooling_sizes[0]) * (n_kers[0])

        self.layers = [
            accelerated_layer.Conv2DAccel(0, 'Conv2DAccel', n_kers[0], ker_sz[0], n_chans, wt_scale, 'relu', reg, r_seed, verbose=verbose),
            accelerated_layer.MaxPool2DAccel(1, 'MaxPool2DAccel', pooling_sizes[0], pooling_strides[0], activation="linear", reg=reg, verbose=verbose),
            layer.Flatten(2, 'Flatten', verbose=verbose),
            layer.Dense(3, 'Dense', dense_interior_units[0], prev_layer, wt_scale=wt_scale, activation="relu", reg=reg, r_seed=r_seed, verbose=verbose),
            layer.Dense(4, 'SoftMax', n_classes, dense_interior_units[0], wt_scale=wt_scale, activation="softmax", reg=reg, r_seed=r_seed, verbose=verbose)
        ]
...
class ConvNet4AccelV2(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(16,), ker_sz=(7,), dense_interior_units=(100,),
                 pooling_sizes=(2,), pooling_strides=(2,), n_classes=10, wt_scale=1e-2, reg=0.0, dropout_rate= 0.0, r_seed=None,
                 verbose=True):
        ...
        self.layers = [
            accelerated_layer.Conv2DAccel(0, 'Conv2D', n_kers[0], ker_sz[0], n_chans, wt_scale, 'relu', reg, r_seed, verbose=verbose),
            accelerated_layer.MaxPool2DAccel(1, 'MaxPool2D', pooling_sizes[0], pooling_strides[0], activation="linear", reg=reg, verbose=verbose),
            layer.Flatten(2, 'Flatten', verbose=verbose),
            layer.Dense(3, 'Dense', dense_interior_units[0], prev_layer, wt_scale=wt_scale, activation="relu", reg=reg, r_seed=r_seed, verbose=verbose),
            layer.Dropout(4, 'Dropout', dropout_rate, r_seed=r_seed, verbose=verbose),
            layer.Dense(5, 'SoftMax', n_classes, dense_interior_units[0], wt_scale=wt_scale, activation="softmax", reg=reg, r_seed=r_seed, verbose=verbose)
        ]
