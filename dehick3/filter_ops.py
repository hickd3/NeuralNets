...
def conv2_gray(img, kers, verbose=True):
    ...
    pad_y = int(np.ceil((ker_y - 1) / 2))
    pad_x = int(np.ceil((ker_x - 1) / 2))

    if verbose:
        print(f'pad_x: {pad_x}, pad_y: {pad_y}')

    padded_img = np.pad(img, ((pad_y, pad_y), (pad_x, pad_x)), mode='constant', constant_values=0)
    filtered_img = np.zeros((n_kers, img_y, img_x))

    for filt in range(n_kers):
        ker= kers[filt, :, :]
        ker= np.flipud(np.fliplr(ker))
        for j in range(img_y):
            for i in range(img_x):
                filtered_img[filt, j, i]= np.sum(padded_img[j:j + ker_y, i:i + ker_x] * ker)
...
def conv2(img, kers, verbose=True):
    ...
    padded_img = np.pad(img, ((0, 0), (pad_y, pad_y), (pad_x, pad_x)), mode='constant', constant_values=0)
    filtered_img = np.zeros((n_kers, n_chans, img_y, img_x))
    ...
    for filt in range(n_kers):
        ...
        for chan in range(n_chans):
            for j in range(img_y):
                for i in range(img_x):
                    filtered_img[filt, chan, j, i] = np.sum(padded_img[chan, j:j + ker_y, i:i + ker_x] * ker)
...
def conv2nn(imgs, kers, bias, verbose=True):
    ...
    padded_img = np.pad(imgs, ((0, 0), (0, 0), (pad_y, pad_y), (pad_x, pad_x)), mode='constant', constant_values=0)
    ...
    for img in range(batch_sz):
        for filt in range(n_kers):
            for chan in range(n_chans):
                ...
                for j in range(img_y):
                    for i in range(img_x):
                        output[img, filt, j, i] += np.sum(padded_img[img, chan, j:j + ker_y, i:i + ker_x] * ker)
            output[img, filt, :, :] += bias[filt]
    return output
...
def max_pool(inputs, pool_size=2, strides=1, verbose=True):
    ...
    if verbose == True:
        print(f'Outputs shape: ({out_y}, {out_x})')
    ...
    for j in range(out_y):
        for i in range(out_x):
            input_y = j * strides
            input_x = i * strides
            window = inputs[input_y:input_y + pool_size, input_x:input_x + pool_size]
            max= np.max(window)
            outputs[j, i] = max
    return outputs
...
def max_poolnn(inputs, pool_size=2, strides=1, verbose=True):
    ...
    if verbose == True:
        print(f'Outputs shape: ({mini_batch_sz}, {n_chans}, {out_y}, {out_x})')

    outputs = np.zeros((mini_batch_sz, n_chans, out_y, out_x))
    for img in range(mini_batch_sz):
        for filt in range(n_chans):
            for j in range(out_y):
                for i in range(out_x):
                    input_y = j * strides
                    input_x = i * strides
                    window = inputs[img, filt, input_y:input_y + pool_size, input_x:input_x + pool_size]
                    max= np.max(window)
                    outputs[img, filt, j, i] = max
    return outputs
