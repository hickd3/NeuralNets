...
class SGD(Optimizer):
    ...
    def update_weights(self):
        ...
        '''
        wts_t= self.wts - self.lr * self.d_wts
        return wts_t.copy()
...
class SGD_Momentum(Optimizer):
    ...
    def update_weights(self):
        ...
        v_t= self.m * self.velocity - self.lr * self.d_wts
        wts_t= self.wts + v_t
        self.velocity = v_t
        self.wts = wts_t
        return wts_t.copy()
...
class Adam(Optimizer):
    ...
    def update_weights(self):
        ...
        self.t += 1
        v_t = self.beta1 * self.v + (1 - self.beta1) * self.d_wts
        p_t = self.beta2 * self.p + (1 - self.beta2) * (self.d_wts ** 2)
        v_t_hat = v_t / (1 - self.beta1 ** self.t)
        p_t_hat = p_t / (1 - self.beta2 ** self.t)
        wts_t = self.wts - self.lr * v_t_hat / (np.sqrt(p_t_hat) + self.eps)
        self.v = v_t
        self.p = p_t
        self.wts = wts_t
        return wts_t.copy()
