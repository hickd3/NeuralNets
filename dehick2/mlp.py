...
class MLP:
    ...
    def predict(self, features):
        ...
        '''
        z= (np.dot(features, self.y_wts))+ self.y_b
        y = np.maximum(0, z)
        y_pred = (np.dot(y, self.z_wts) + self.z_b)
    ...
    def forward(self, features, y, reg=0):
        ...
        z_net_act = np.exp(z_net_in) / np.sum(np.exp(z_net_in), axis=1, keepdims=True)

        y_one_hot = self.one_hot(y, self.num_output_units)
        loss = -np.sum(y_one_hot * np.log(z_net_act)) / features.shape[0]
        loss += reg * (np.sum(self.y_wts ** 2) + np.sum(self.z_wts ** 2)) / 2
    ...
    def backward(self, features, y, y_net_in, y_net_act, z_net_in, z_net_act, reg=0):
        ...
        dz_net_act = (z_net_act - y_one_hot) / features.shape[0]

        dz_wts = np.dot(y_net_act.T, dz_net_act) + reg * self.z_wts
        dz_b = np.sum(dz_net_act, axis=0)

        dy_net_act = np.dot(dz_net_act, self.z_wts.T)
        dy_net_act[y_net_in <= 0] = 0

        dy_wts = np.dot(features.T, dy_net_act) + reg * self.y_wts
        dy_b = np.sum(dy_net_act, axis=0) 
    ...
    def fit(self, features, y, x_validation, y_validation, n_epochs=500, lr=0.0001, mini_batch_sz=256, reg=0,
        ...
        '''
        if r_seed is not None:
            np.random.default_rng(r_seed)
        else:
            np.random.default_rng()
        
        self.initialize_wts(features.shape[1], self.num_hidden_units, self.num_output_units, r_seed=r_seed)

        num_samps = features.shape[0]
        num_batches = num_samps // mini_batch_sz
        loss_history = []
        train_acc_history = []
        validation_acc_history = []
        #print("Training samples:", features.shape[0])
        
        if verbose > 0:
            print(f"Starting to train network...There will be {n_epochs} epochs and {n_epochs * num_batches} iterations total, {num_batches} iter/epoch.")
        iter_count = 0
        for epoch in range(n_epochs):
            i = np.arange(num_samps)
            np.random.shuffle(i)
            features = features[i]
            y = y[i]
            
            for i in range(num_batches):
                start = i * mini_batch_sz
                end = (i + 1) * mini_batch_sz
            ...
            if verbose > 0 and epoch % print_every == 0:
                print(f"Completed epoch {epoch}/{n_epochs -1}. Training loss: {loss:.2f}. Training acc: {train_acc:.2f}. Validation acc: {validation_acc:.2f}.")
